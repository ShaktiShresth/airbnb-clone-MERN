    ...
    return (
      <div className="bg-gray-100 relative">
        ...
        <div className="px-4 py-8 lg:px-20 grid gap-y-8 gap-x-6 grid-cols-2 max-[430px]:grid-cols-1 md:grid-cols-3 lg:grid-cols-4">
          {
             searchTerm? places.filter((place) => {
              if(searchTerm == ""){
                  return  place;
              }else if(place.address.toLowerCase().includes(searchTerm.toLowerCase()) || place.title.toLowerCase().includes(searchTerm.toLowerCase())){
                  return place;
              }
            }).map((place, index) => {
              return (
                  ...
                  <div className="bg-gray-500 rounded-2xl flex mb-2">
                    {place.photos?.[0] && (
                      <img className="rounded-2xl object-cover aspect-square hover:scale-105" src={'http://localhost:4000/uploads/' + place.photos?.[0]} alt=""/>
                    )}
                  </div>
                  <h2 className="font-bold truncate">{place.address}</h2>
                  <h3 className="text-gray-500 text-sm truncate">{place.title}</h3>
                  <div className="text-gray-500 mt-1">
                    <span className="text-black font-bold">${place.price}</span> night
                  </div>
                </Link>
              )
            }) 
            ...
            visiblePlaces.map((place, index) => {
              return (
                <Link to={'/place/'+place._id} key={index}>
                    <div className="bg-gray-500 rounded-2xl flex mb-2">
                    {place.photos?.[0] && (
                      <img className="rounded-2xl object-cover aspect-square hover:scale-105 transition duration-200 ease-in hover:delay-150" src={'http://localhost:4000/uploads/' + place.photos?.[0]} alt=""/>
                    )}
                  </div>
                  <h2 className="font-bold truncate">{place.address}</h2>
                  <h3 className="text-gray-500 text-sm truncate">{place.title}</h3>
