...
export default function ProfilePage() {
  ...
  // using parsedData from localStorage instead of user from contextApi
  if (ready && !parsedData && !redirectHomePage) {
      ...
      {subpage === "profile" && (
        <div className="py-10 md:py-20 flex flex-col items-center justify-center mx-auto mt-8 bg-gradient-to-r from-gray-100 to-gray-200 rounded-2xl">
          <svg
            ...
            strokeWidth={0.9}
            stroke="currentColor"
            className="w-44 h-44 -mt-4 md:w-60 md:h-60"
          >
            <path
              strokeLinecap="round"
          ...
          <button
            onClick={logout}
            className="py-2 w-40 rounded-full bg-primaryColor text-white mt-4 transition delay-100 ease-in hover:bg-black text-sm md:text-base"
          >
            Logout
          </button>
        </div>
